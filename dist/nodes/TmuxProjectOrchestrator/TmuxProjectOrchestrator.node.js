...
Begin coordinating with your QA Engineer for the development workflow.`;
            ...
            await bridge.sendClaudeMessage(`${sessionName}:1`, `PROJECT PLAN APPROVED: Development starting now.
...
exit 0`;
    ...
    static async handleGetStatus(context, itemIndex, bridge) {
        ...
        try {
            ...
            const statusRequests = session.windows.map(async (window, index) => {
                await bridge.sendClaudeMessage(`${sessionName}:${window.windowIndex}`, 'STATUS REQUEST: Provide current work status, progress, and any blockers.');
            });
    ...
    static async handleGetAllProjectsStatus(context, itemIndex, bridge) {
        try {
            const sessions = await bridge.getTmuxSessions();
...
Please acknowledge receipt and provide ETA.`;
            await bridge.sendClaudeMessage(`${sessionName}:0`, `TASK DELEGATION REQUEST:\n${taskMessage}\n\nPlease assign this task to appropriate team member and coordinate execution.`);
            if (targetAgent) {
                try {
                    const sessions = await bridge.getTmuxSessions();
                    const session = sessions.find(s => s.name === sessionName);
                    if (session) {
                        const targetWindow = session.windows.find(w => w.windowName.toLowerCase().includes(targetAgent.toLowerCase()) ||
                            w.windowIndex.toString() === targetAgent);
                        if (targetWindow) {
                            await bridge.sendClaudeMessage(`${sessionName}:${targetWindow.windowIndex}`, taskMessage);
                        }
                    }
                }
                catch (e) {
            ...
            if (requiresQA) {
                ...
                if (qaWindow) {
                    await bridge.sendClaudeMessage(`${sessionName}:${qaWindow.windowIndex}`, `📋 QA ALERT: New task scheduled requiring validation.
Task ID: ${taskId}
...
Remember: All commits require QA approval from our QA Engineer.`);
            const qaWindow = session.windows.find(w => w.windowName.toLowerCase().includes('qa') || w.windowIndex === 1);
            if (qaWindow) {
                await bridge.sendClaudeMessage(`${sessionName}:${qaWindow.windowIndex}`, `👋 NEW TEAM MEMBER: ${memberRole} joined the project (Window ${newWindowIndex}).
...
Please redistribute any pending tasks to remaining team members.`);
    ...
    static async conductStandup(bridge, session) {
        ...
        try {
            for (const window of session.windows) {
                await bridge.sendClaudeMessage(`${sessionName}:${window.windowIndex}`, `📊 DAILY STANDUP: Please provide your status update:
...
You may now proceed with git operations.`);
    ...
    static async handleBlockCommit(context, itemIndex, bridge) {
        ...
        try {
            ...
            await bridge.sendClaudeMessage(`${sessionName}:0`, `❌ QA REJECTION: Git commits are BLOCKED.
Commit Message: ${commitMessage}
...
Please address the QA concerns and request new testing.`);
    ...
    static generateStatusRecommendations(teamStatus, qaApproved) {
        ...
        const blockedAgents = teamStatus.filter(t => t.status.toLowerCase().includes('blocked') ||
...
Performance is a quality attribute - ensure all performance work is QA validated.`,
    ...
    static assessProjectHealth(windows, qaApproved) {
        ...
        if (!hasPM)
            issues.push('Missing Project Manager');
        if (!hasQA)
            issues.push('Missing QA Engineer');
        if (!qaApproved)
            issues.push('Commits blocked - QA approval needed');
        let health = 'good';
        if (issues.length > 2) {
            health = 'critical';
        }
    ...
    static generateSystemRecommendations(runningProjects, _finishedProjects) {
        const recommendations = [];
    ...
    static calculateSystemHealth(runningProjects) {
